...
# X=np.vstack((X1, X2))
# y= np.vstack((y1, y2))
X = np.load("老数据/X.npy", allow_pickle=True)
y = np.load("老数据/y.npy", allow_pickle=True)
...
def LightweightUNetDS16(length, n_channel=1):
    x = 8  # Reduced number of filters for lightweight model
...
def MultiResUNet1D(length, n_channel=1):
    ...
    mresblock1 = MultiResBlock(16, inputs)  # Reduced number of filters for lightweight model
    pool1 = MaxPooling1D(pool_size=2)(mresblock1)
    mresblock1 = ResPath(16, 2, mresblock1)  # Reduced ResPath length

    mresblock2 = MultiResBlock(32, pool1)
    pool2 = MaxPooling1D(pool_size=2)(mresblock2)
    mresblock2 = ResPath(32, 2, mresblock2)  # Reduced ResPath length

    mresblock3 = MultiResBlock(64, pool2)
    pool3 = MaxPooling1D(pool_size=2)(mresblock3)
    mresblock3 = ResPath(64, 1, mresblock3)  # Reduced ResPath length

    mresblock4 = MultiResBlock(128, pool3)

    up6 = concatenate([UpSampling1D(size=2)(mresblock4), mresblock3], axis=-1)
    mresblock6 = MultiResBlock(64, up6)

    up7 = concatenate([UpSampling1D(size=2)(mresblock6), mresblock2], axis=-1)
    mresblock7 = MultiResBlock(32, up7)

    up8 = concatenate([UpSampling1D(size=2)(mresblock7), mresblock1], axis=-1)
    mresblock8 = MultiResBlock(16, up8)

    conv10 = Conv1D(1, 1, activation='linear')(mresblock8)
...
# 训练模型
history = model.fit(X_train, y_train, epochs=100, batch_size=50, validation_data=(X_val, y_val), callbacks=[checkpoint, metrics_callback])
